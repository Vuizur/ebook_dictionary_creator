...
def add_word_if_does_not_exist(cur: Cursor, canon_form: str):
    ...
    res = cur.execute("SELECT * FROM word WHERE canonical_form == ? OR alternative_canonical_form == ?",
                      (canon_form, canon_form)).fetchone()
    if res == None:
        cur.execute("INSERT INTO word (canonical_form, word, word_lowercase, word_lower_and_without_yo) VALUES (?, ?, ?, ?)",
...
def delete_inconsistent_canonical_forms(db_path):
    ...
    cur = con.cursor()
    res = cur.execute(
        "SELECT word_id, canonical_form, alternative_canonical_form, word FROM word").fetchall()
    ...
    if len(form_tags) == 2 and (("nominative" in form_tags and "plural" in form_tags) or ("genitive" in form_tags and "plural")):
        ...
        for form_tag in form_tags:
            if form_tag == "canonical" and form_dict["canonical_form"] == None:
                ...
                form_dict["alternative_canonial_form"] = remove_weird_characters_for_alternative_canonical(
                    word_form)
...
def add_inflection_to_db(cur: Cursor, infl_str, base_word_pos, base_word_id, infl_tags):
    ...
    cur.execute("INSERT INTO form_of_word (word_id, base_word_id) VALUES (?, ?)",
                (word_id, base_word_id))
    fow_id = cur.lastrowid
    for tag in infl_tags:
        cur.execute(
            "INSERT INTO case_tags (form_of_word_id, tag_text) VALUES (?, ?)", (fow_id, tag))
...
def create_database_russian(database_path: str, wiktextract_json_path: str):
    ...
    with open(wiktextract_json_path, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            for sense in obj["senses"]:
                ...
                if "form_of" in sense:
                    ...
                    cur.execute(
                        "INSERT INTO sense (word_id) VALUES (?)", (word_id,))
                    ...
                    try:
                        for gloss in sense["glosses"]:
                            if DO_NOT_ADD_TRANSLATIONS:
                                continue
                            cur.execute(
                                "INSERT INTO gloss (sense_id, gloss_string, gloss_lang, gloss_source) VALUES(?, ?, ?, ?)", (sense_id, gloss, "en", "wi"))
                    except:
                        pass
        ...
        cur.execute(
            "CREATE INDEX canfor_pos_index ON word(canonical_form, pos);")
        cur.execute(
            "CREATE INDEX wordlo_pos_index ON word(word_lowercase, pos);")
        cur.execute(
            "CREATE INDEX wordlo_noyo_index ON word(word_lower_and_without_yo, pos);")

        cur.execute(
            "CREATE INDEX word_lowercase_index ON word(word_lowercase);")
        cur.execute(
        ...
        cur.execute(
            "CREATE INDEX base_word_id_index ON form_of_word(base_word_id);")
        cur.execute(
        ...
        for base_word_id, infl_str, infl_tags, base_word_pos in inflections:
            ...
            if "(" in infl_str and ")" in infl_str:
                ...
                add_inflection_to_db(
                    cur, word1, base_word_pos, base_word_id, infl_tags)
                add_inflection_to_db(
                    cur, word2, base_word_pos, base_word_id, infl_tags)
                ...
                already_added_tagged_infls[unaccentify(
                    infl_str)] = set(infl_tags)
