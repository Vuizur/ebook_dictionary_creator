...
def create_kindle_dict(
    ...
    title: str,
    kindlegen_path,
    try_to_fix_kindle_lookup_stupidity=False,
    tatoeba_path=None,
):
...
WHERE w2.word = ?""",
        ...
        for inflection in infl_list:
            ...
            if (
                try_to_fix_kindle_lookup_stupidity
                and (
                    not unidecoded_inflection
                    in already_separated_unidecoded_inflections
                )
                and (
                    (unidecoded_inflection.lower() in base_forms_unidecoded)
                    or (inflection_counts[inflection] > 1)
                )
            ):
    ...
    if tatoeba_path != None:
        ...
        tatoeba_words = get_word_and_html_for_all_words_not_in_word_list(
            existing_words, "Sentence pairs in Czech-English - 2022-06-15.tsv"
        )
        for word, html in tatoeba_words.items():
            glos.addEntryObj(
                glos.newEntry(["HTML_HEAD<b>" + word + "</b>", word], html, defiFormat)
            )
