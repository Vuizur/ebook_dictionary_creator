...
from ebook_dictionary_creator.e_dictionary_creator.create_kindle_dict import create_kindle_dict
from ebook_dictionary_creator.e_dictionary_creator.create_tab_file import create_nonkindle_dict

class DictionaryCreator:

    # Initialize the class with the source language and target language
    def __init__(self, source_language: str, target_language: str, kaikki_file_path = None, database_path = None):
    ...
    def download_data_from_kaikki(self, kaikki_file_path: str = None):
        ...
        with open(kaikki_file_path, "w") as f:
            f.write(requests.get(f"https://kaikki.org/dictionary/{self.source_language}/kaikki.org-dictionary-{self.source_language}.json").text)
        self.kaikki_file_path = kaikki_file_path
    ...
    def create_database(self, database_path: str = None):
        ...
        create_database.create_database(database_path, self.kaikki_file_path, self.source_language)    
        self.database_path = database_path
    ...
    def export_to_stardict(self, author: str, title: str, stardict_path: str = None):
        ...
        create_nonkindle_dict(self.database_path, stardict_path, "Stardict", self.source_language, self.target_language, author, title)
        self.stardict_path = stardict_path


    def export_to_kindle(self, kindlegen_path: str, try_to_fix_failed_inflections: str, author: str, title: str, mobi_path: str = None,):
