...
class TatoebaAugmenter():
    ...
    def __init__(self, source_language: str, target_language: str, exception_word_list: set[str] = []):
        ...
        self.parallel_corpus = ParallelCorpus(source_language_code, target_language_code)
    ...
    def create_word_occurence_dictionary(self):
        ...
        for sentence, _ in self.parallel_corpus:
            ...
            words = wordfreq.tokenize(sentence.text, self.get_language_code(self.source_language))
    ...
    def get_example_sentences_for_all_words_not_in_word_list(self
        ...
        for word in new_unique_words_dict.keys():
            ...
            for sentence_id in new_unique_words_dict[word]:
                ...
                words_with_sentence_examples[word].append((text_sentence, text_translation))
                # Print a message every 5000 sentences
                if len(words_with_sentence_examples[word]) % 5000 == 0:
                    print(f"{len(words_with_sentence_examples[word])} sentences have been processed.")
