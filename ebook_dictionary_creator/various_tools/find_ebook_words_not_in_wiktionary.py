                    ...
                    for text_object in soup.find_all(text=True):
                        ...
                        if len(text) > 0:
                            ...
                            for token in doc:
                                ...
                                if language == "Russian":
                                    res = cur.execute(
                                        "SELECT word FROM word WHERE word_lowercase = ?",
                                        (token,),
                                    ).fetchone()
                                    if res == None:
                                        res = cur.execute(
                                            "SELECT word FROM word WHERE word_lower_and_without_yo = ?",
                                            (token,),
                                        ).fetchone()
                                else:
                                    # This has the problem that abbreviations like FBI don't get found!
                                    res = cur.execute(
                                        "SELECT word FROM word WHERE word = ?", (token,)
                                    ).fetchone()
                                    if res == None:
                                        res = cur.execute(
                                ...
                                if res == None:
                                    if token not in word_histogram:
                                        ...
                                        word_histogram[token] = (
                                            word_histogram[token] + 1
                                        )
    rmtree(extract_dir)
    word_histogram = dict(
        (sorted(word_histogram.items(), key=lambda item: item[1], reverse=True))
    )
