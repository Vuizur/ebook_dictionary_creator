...
def find_RAE_words_not_in_Wiktionary():
    with open("diccionario-rae-completo.txt", encoding="utf-8") as f, open("words_in_RAE_not_in_Wiktionary.txt", "w", encoding="utf-8") as out:
        conn = sqlite3.connect("compiled_databases/Spanish_dict.db")

        for l in f.readlines():
            res = conn.execute("SELECT word FROM word WHERE word = ? OR word = ? OR word = ?", (l.lower().strip(), l.capitalize().strip(), l.upper().strip())).fetchone()
            if res == None:
                out.write(l)

def order_RAE_words_not_in_Wiktionary():
    with open("words_in_RAE_not_in_Wiktionary.txt", encoding="utf-8") as input, \
        open("CREA_total.txt", encoding="utf-8") as sorted_input:
    ...
    with open("words_in_RAE_not_in_Wiktionary_sorted.txt", "w", encoding="utf-8") as sorted_RAE:
        finalized_ordered_words = []
        for word in RAE_words_frequency:
            try:
                index_of_word = missing_words_alphabet_sort_lower.index(word)
                finalized_ordered_words.append(missing_words_alphabet_sort[index_of_word])
